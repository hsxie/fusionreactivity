...
for j=1:length(TT)
    ...
    N=10000; N3=40*N; N2=8*floor(sqrt(N));
    tmp1=cputime;
    ...
    tmp2=cputime;
    [sgmv2,stdsgmv2]=fsgmvmcdtm(vt1x,vt1y,vt1z,vd1x,vd1y,vd1z,vt2x,vt2y,vt2z,vd2x,vd2y,vd2z,N2,2);
    tmp2=cputime-tmp2; runtime2=runtime2+tmp2;
    tmp3=cputime;
    [sgmv3,stdsgmv3]=fsgmvmcdtm(vt1x,vt1y,vt1z,vd1x,vd1y,vd1z,vt2x,vt2y,vt2z,vd2x,vd2y,vd2z,N3,3);
...
subplot(131);
plot(Tr1d,sgmv1d_all(jE,:),':','linewidth',3); hold on;
errorbar(TT,sgmvv1,stdsgmvv1,'.','linewidth',2); hold on;
xlabel('T_r [keV]'); ylabel('<\sigma{}v> (m^3/s)');
hleg1=legend('1D analytical',['method=1, using f_{1,2}, O(N),',...
...
text(20,0.5e-21,['R_t=',num2str(Rt),', E_d=',num2str(EdkeV),'keV,',...
...
subplot(132);
plot(Tr1d,sgmv1d_all(jE,:),':','linewidth',3); hold on;
errorbar(TT,sgmvv2,stdsgmvv2,'s','linewidth',2); hold on;
xlabel('T_r [keV]'); %ylabel('<\sigma{}v> (m^3/s)');
hleg2=legend('1D analytical',['method=2, using f_{1,2}, O(N^2),',...
...
xlabel('T_r [keV]'); %ylabel('<\sigma{}v> (m^3/s)');
hleg3=legend('1D analytical',['method=3, using Gaussian ',10,...
...
% print(gcf,'-dpdf',['smgv_mc_dtm_Rt=',num2str(Rt),'_Ed=',num2str(EdkeV),'_N1=',num2str(N),...
%     '_N2=',num2str(N2),'_N3=',num2str(N3),'.pdf']);
print(gcf,'-dpng',['smgv_mc_dtm_Rt=',num2str(Rt),'_Ed=',num2str(EdkeV),'_N1=',num2str(N),...
